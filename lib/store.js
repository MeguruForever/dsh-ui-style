...
export function createStyleStore(root = defaultSkillsRoot()) {
  ...
  return {
    ...
    async remove(name) {
      ...
      // Clearing the active selection when its style disappears keeps the
      // composer chip honest.
      const selection = await this.getSelection();
      if (selection === name) await this.setSelection(null);
      return true;
    },

    /** The style currently pinned for quick use in the composer, or null. */
    async getSelection() {
      try {
        const parsed = JSON.parse(await fs.readFile(join(skillsRoot, 'selection.json'), 'utf8'));
        return typeof parsed.name === 'string' && isValidSkillName(parsed.name) ? parsed.name : null;
      } catch {
        return null;
      }
    },

    /** Pin (or clear, with null) the style offered in the composer dock. */
    async setSelection(name) {
      await ensureRoot();
      const file = join(skillsRoot, 'selection.json');
      if (name === null) {
        await fs.rm(file, { force: true });
        return;
      }
      safeChild(skillsRoot, name); // validation only
